...
enum QuotaManager {
    // Daily free limits (same as app/share/web)
    ...
    // Use the App Group so all targets share quota counters
    private static let groupID = "group.com.conornolan.screenactions"
    ...
    /// Checks and, if allowed, consumes one unit from today's quota.
    static func consume(feature: SAFeature, isPro: Bool) -> QuotaResult {
        ...
        if used >= limit {
            let msg: String = {
                switch feature {
                case .receiptCSVExport:
                    return "You’ve hit today’s free limit (3 CSV exports). Go Pro for unlimited."
                case .createContactFromImage:
                    return "You’ve hit today’s free limit (5 contact saves from images). Go Pro for unlimited."
                case .geofencedEventCreation:
                    return "You’ve hit today’s free limit (1 geofenced event). Go Pro for unlimited."
                }
            }()
