...
//  Pure + cheap (NSDataDetector + regex). Adds a “place-like” boost so
//  bare place names (e.g. “Dublin Castle”) route to Event.
//

import Foundation

enum ScreenActionKind: String { case reminder, event, contact, receipt }
...
enum ActionRouter {
    ...
    static func route(text raw: String, locale: Locale = .current) -> RouteDecision {
        ...
        var scores: [ScreenActionKind: Int] = [
            .reminder: 0, .event: 0, .contact: 0, .receipt: 0
        ]
        var reasons: [String] = []
        ...
        if let currencyRegex {
            for line in lines {
                ...
                if currencyRegex.firstMatch(in: line, range: range) != nil {
                    currencyHits += 1
                }
            }
        }
        if currencyHits >= 2 { scores[.receipt, default: 0] += 4; reasons.append("≥2 currency lines") }
        if text.localizedCaseInsensitiveContains("subtotal")
            || text.localizedCaseInsensitiveContains("total")
            || text.localizedCaseInsensitiveContains("vat")
            || text.localizedCaseInsensitiveContains("tax")
        ...
        if let linkDetector {
            ...
            linkDetector.enumerateMatches(in: text, options: [], range: r) { match, _, _ in
                ...
                if url.scheme?.lowercased() == "mailto" || url.absoluteString.contains("@") {
                    emailCount += 1
                }
            }
        } else {
        ...
        if containsAny(text, [
            "meeting","call","dinner","flight","concert","appointment","appt",
            "zoom","teams","google meet","rsvp","from","to","at"
        ]) {
            scores[.event, default: 0] += 1
        }

        if containsAny(text, [
            "remind me","remember to","todo","to-do","buy","pick up","call",
            "email","pay","renew","follow up","due","by "
        ]) {
            scores[.reminder, default: 0] += (dateRange == nil ? 3 : 1)
        }

        // NEW: place-only heuristic → nudge towards Event (for geofencing editor)
        if dateRange == nil && looksLikeStandalonePlace(text) {
            scores[.event, default: 0] += 3
            reasons.append("looks like place")
        }
    ...
    private static func containsAny(_ haystack: String, _ needles: [String]) -> Bool {
        for w in needles where haystack.localizedCaseInsensitiveContains(w) {
            return true
        }
        return false
    }

    /// True for short, single-line, digit-less, proper-noun-ish inputs that contain common place words.
    private static func looksLikeStandalonePlace(_ text: String) -> Bool {
        let t = text.trimmingCharacters(in: .whitespacesAndNewlines)
        if t.isEmpty { return false }
        if t.contains("\n") { return false }
        if t.count < 3 || t.count > 64 { return false }
        if t.rangeOfCharacter(from: .decimalDigits) != nil { return false }

        let lowered = t.lowercased()
        let placeKeywords = [
            "castle","park","airport","station","hotel","museum","university","college",
            "centre","center","hospital","clinic","pub","restaurant","cafe","bar","library",
            "street"," st","road"," rd","avenue"," ave","lane"," ln","square"," sq","mall",
            "court","stadium","arena","office","building","hall","theatre","theater","cinema",
            "gallery","campus","pier","harbour","harbor","port","bridge","cathedral","church",
            "temple","mosque","abbey","fort","fortress"
        ]
        if placeKeywords.contains(where: { lowered.contains($0) }) { return true }

        let words = t.split { $0 == " " || $0 == "\t" }
        if (1...4).contains(words.count) {
            let isTitleCase = words.allSatisfy { w in
                guard let f = w.first else { return false }
                return String(f).uppercased() == String(f)
            }
