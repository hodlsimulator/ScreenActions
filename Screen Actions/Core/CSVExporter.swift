...
enum CSVExporter {
    ...
    /// Main-actor isolated because it touches `AppStorageService.shared` (which is @MainActor).
    @MainActor
    static func writeCSVToAppGroup(filename: String, csv: String) throws -> URL {
        let base = AppStorageService.shared
            .containerURL()
        ...
        guard let data = csv.data(using: .utf8) else {
            throw NSError(domain: "CSVExporter", code: 1, userInfo: [NSLocalizedDescriptionKey: "UTF-8 encoding failed."])
        }
