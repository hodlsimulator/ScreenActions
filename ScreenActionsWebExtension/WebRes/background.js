// background.js — Safari-safe native bridge + selection cache.
// Works on iOS/macOS Safari (WebExtensions), Chrome, and Firefox.
// Key change: getPageCtx now falls back to a live snapshot so the popup
// shows the selection on first open, every time.
...
(() => {
  ...
  const SCRIPTING =
    (typeof chrome  !== 'undefined' && chrome.scripting)  ? chrome.scripting  :
    (typeof browser !== 'undefined' && browser.scripting) ? browser.scripting :
    undefined;

  if (!RT) return;

  // -------- Per-tab context cache (fed by content_selection.js) --------
  // Shape: { selection, title, url, structured?, ts }
  const sel = new Map();
  ...
  // -------- On-demand snapshot via scripting --------
  // Works even if the content script didn’t inject (e.g., no host perms).
  ...
  // Merge any provided payload with cache, and if still missing selection, grab a live snapshot.
  async function enrichPayloadWithSnapshot(payload, tabIdMaybe) {
    ...
    // If selection still empty, try a live snapshot (allowed thanks to user gesture: opening the popup).
  ...
  const withTimeout = (p, ms) => new Promise((res, rej) => {
    ...
    Promise.resolve(p).then(v => { clearTimeout(t); res(v); })
  ...
  async function sendNative(message) {
    let msg;
    try { msg = JSON.parse(JSON.stringify(message || {})); } catch { msg = {}; }
    ...
    try {
      const r = f(msg);
      if (r?.then) return await withTimeout(r, TIMEOUT);
    } catch {}
      ...
      return await withTimeout(new Promise((res, rej) => {
        try {
          f(msg, (out) => {
            ...
            if (le) rej(new Error(le.message || 'Native error'));
            else res(out);
          });
        } catch (e) { rej(e); }
      }), TIMEOUT);
  ...
  async function handle(request, sender) {
    ...
    if (cmd === 'selUpdate') {
      return setCtx(sender, request.payload || {});
    }

    if (cmd === 'getPageCtx') {
      // NEW: use live snapshot fallback so first-open shows selection.
      const enriched = await enrichPayloadWithSnapshot({}, sender?.tab?.id);
      return {
        ok: true,
        ctx: {
    ...
    if (cmd === 'native') {
      ...
      // Ensure we send real selection/title/url (pull live snapshot if needed).
