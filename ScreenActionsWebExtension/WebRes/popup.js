...
(() => {
  ...
  const SCR = (typeof browser !== 'undefined' && browser.scripting)
    ? browser.scripting
  ...
  function fillContactFields(f){
    ...
    el('ctStreet').value=f.street||''; el('ctCity').value=f.city||''; el('ctState').value=f.state||''; el('ctPostal').value=f.postalCode||''; el('ctCountry').value=f.country||'';
  }
  ...
  function gatherContactFields(){
    ...
    return {givenName,familyName,emails,phones,
      street:el('ctStreet').value,city:el('ctCity').value,state:el('ctState').value,postalCode:el('ctPostal').value,country:el('ctCountry').value};
  }
