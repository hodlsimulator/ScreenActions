...
for f in glob.glob(os.path.expanduser("~/Library/MobileDevice/Provisioning Profiles/*.mobileprovision")):
    ...
    extids=ent.get("com.apple.developer.extensionkit.extension-point-identifiers") or []
    if "com.apple.Safari.web-extension" not in extids: continue
    ...
    cands.append({"name":d.get("Name") or "", "uuid":d.get("UUID") or "", "team":team, "bundle_id":bid})
if not cands:
